...
Pode-se entrar com dados no R de diferentes formas. O formato mais
adequado vai depender do tamanho do conjunto de dados, e se os dados já
existem em outro formato para serem importados ou se serão digitados
diretamente no R.

A seguir são descritas formas de entrada de dados com
...
endereços de URLs (endereços da *web*) o que iremos mencionar em mais
...
1. Usnado a função `scan()` crie objetos para armazenar os seguintes
valores:
...
- Se houverem valores perdidos, preencha com `NA`s
- A matriz de dados deve formar um bloco só. Se houverem colunas de
  diferentes comprimentos, preencha com `NA`s
...
função `read.table()`. Para importar um arquivo `.csv` faça:
...
1. Baixe os seguintes arquivos:
   - [gam01.txt](http://www.leg.ufpr.br/~fernandomayer/dados/gam01.txt)
   - [exemplo02.txt](http://www.leg.ufpr.br/~fernandomayer/dados/exemplo02.txt)
   - [dadosfic.csv](http://www.leg.ufpr.br/~fernandomayer/dados/dadosfic.csv)
   - [compingest.txt](http://www.leg.ufpr.br/~walmes/data/compingest.txt)
   - [biomassa.txt](http://www.leg.ufpr.br/~walmes/data/biomassa.txt)

Coloque os arquivos em um local apropriado (de preferência no mesmo
diretório de trabalho que voce definiu no início da sessão), faça a
importação usando a função `read.table()`, e confira a estrutura dos
dados com `str()`.
...
1. Leia os seguintes conjuntos de dados disponíves *on-line*:
   - <http://www.leg.ufpr.br/~walmes/data/banana.txt>
   - <http://www.leg.ufpr.br/~walmes/data/triathlon.txt>

usando a função `read.table()`, e confira a estrutura dos dados com
`str()`.
...
**todod** os objetos criados na sua área de trabalho (isso inclui
