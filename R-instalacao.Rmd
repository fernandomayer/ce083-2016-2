...
1. <http://cran-r.c3sl.ufpr.br/bin/windows/base/R-3.3.1-win.exe>

2. <https://download1.rstudio.org/RStudio-0.99.903.exe>
...
6.  Clique sobre o link `Download R 3.3.1 for Windows` para fazer o
    download do arquivo `R-3.3.1-win.exe`.
...
5.  Será exibida uma página com a recomendação para você baixar o
    `RStudio 0.99.903 - Windows Vista/7/8`
...
Note que o arquivo `rstudio-0.99.903-amd64.deb` é uma versão específica
...
4.  Na seção `Source Code for all Platforms`, baixe o arquivo
    `R-3.3.1.tar.gz` (link direto:
    <http://cran-r.c3sl.ufpr.br/src/base/R-3/R-3.3.1.tar.gz>)
...
Isso irá criar um diretório chamado `R-3.3.1`. Entre nesse diretório
...
Após a instalação, você pode abrir o RStudio pelo ícone (deoendendo da
